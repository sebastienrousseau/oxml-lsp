//! A JSON value, parser and serialiser, sufficient for LSP.
//!
//! **This is a copy of `oxml-mcp`'s `json.rs`**, taken because both
//! crates speak JSON-RPC and neither can reach the other's private
//! module. It is not a good long-term answer: a defect fixed here will
//! not reach the copy, which is the failure mode this suite keeps
//! finding elsewhere. Extracting a shared crate is filed as an issue.
//!
//! It was copied rather than rewritten because the original is already
//! fuzzed — `oxml-mcp` runs a `handle_line` target over it on every
//! pull request — and a fresh hand-written JSON parser would start
//! with none of that.
//!
//! LSP speaks JSON-RPC over stdio. The messages involved are small and
...
mod tests {
    ...
    fn integral_numbers_print_without_a_fraction() {
        // LSP ids are compared by clients; `2.0` where `2` was sent is
